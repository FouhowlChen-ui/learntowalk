                ...
                if hasattr(field_type, "__dataclass_fields__"):
                    if isinstance(data[field_name], dict):
                        setattr(
                            instance,
                            field_name,
                            DictionableDataclass.create(field_type, data[field_name]),
                        )
                    else:
    ...
    def add_arguments(instance: Any, parser: Any, prefix: str = "") -> None:
        for field in fields(instance):
            ...
            if hasattr(field_type, "__dataclass_fields__"):
                ...
                DictionableDataclass.add_arguments(
                    nested_instance, parser, prefix=f"{field_name}."
                )
            else:
                if field_type == bool:
                    parser.add_argument(
    ...
    def set_from_args(instance: Any, args: Any, prefix: str = "") -> None:
        for field in fields(instance):
            ...
            if hasattr(field_type, "__dataclass_fields__"):
                ...
                DictionableDataclass.set_from_args(
                    nested_instance, args, prefix=f"{prefix}{field_name}."
                )
            else:
